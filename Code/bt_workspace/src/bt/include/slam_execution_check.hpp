 ...
 * 
 */
class SlamExecutionCheck : public BT::ConditionNode
{
public:

    SlamExecutionCheck(const std::string& name) : BT::ConditionNode(name, {})
    {   
        node_= rclcpp::Node::make_shared("slam_execution_check");
        if (debug)
            RCLCPP_INFO(node_->get_logger(), "Constructor SlamExecutionCheck");
        ...
        if (debug)
            RCLCPP_INFO(node_->get_logger(), "Constructor SlamExecutionCheck finished.");
        // debug = true;
        debug = false;
    }
